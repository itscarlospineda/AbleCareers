...
<div>
  <div class="col-md-12">
      <div class="card">
          <div class="card-body">
            <form id="notificationForm"  action="{{ route('notification.create', ['id' => $resume->id, 'idJobPos' =>$idJobPos ]) }}" method="POST">
                  @csrf
                  <!-- Campo oculto para enviar el ID del currículum -->
...
<script>
  ...
  // Función para redirigir al hacer clic en el botón "OK"
  document.getElementById('okButton').addEventListener('click', function() {
    // Envía el formulario
    document.getElementById('notificationForm').submit();
    //window.location.href = "{{ route('jobPosition.recruiterShowPost') }}";
  });
</script>
